...
function addFavorite() {
    if (!startMarker || !endMarker) {
        alert("출발지와 도착지를 모두 설정한 후 저장하세요.");
        return;
    }

    const startName = document.getElementById('startSearch').value;
    const endName = document.getElementById('endSearch').value;
...
function updateFavoritesList() {
    ...
    favorites.forEach((favorite, index) => {
        ...
        li.textContent = `${favorite.start.name} → ${favorite.end.name}`;

        // 삭제 버튼 생성 및 클래스 추가
        const deleteBtn = document.createElement('button');
        deleteBtn.textContent = '삭제';
        deleteBtn.classList.add('delete-btn'); // 클래스 추가
        deleteBtn.addEventListener('click', () => removeFavorite(index));
...
function removeFavorite(index) {
    favorites.splice(index, 1); // 선택한 즐겨찾기 삭제
    updateFavoritesList(); // 리스트 업데이트
...
function loadFavorite(index) {
    ...
    document.getElementById('startSearch').value = favorite.start.name;
    document.getElementById('endSearch').value = favorite.end.name;

    startMarker = new google.maps.Marker({
        position: favorite.start.latLng,
        map: map,
        title: '시작점',
        label: 'S'
    });
    ...
    endMarker = new google.maps.Marker({
        position: favorite.end.latLng,
        map: map,
        title: '도착점',
        label: 'E'
    });

    map.setCenter(favorite.start.latLng);
    getRoute(startMarker.getPosition(), endMarker.getPosition());
}
// 엔터키 이벤트 리스너 추가 함수
...
function toggleFavorites() {
    ...
    const button = document.getElementById('toggleFavoritesButton');

    if (container.style.display === 'none') {
        container.style.display = 'block';
        button.textContent = '즐겨찾기 닫기';
    } else {
        container.style.display = 'none';
        button.textContent = '즐겨찾기 열기';
    }
}

// 버튼에 이벤트 리스너 등록
document.getElementById('toggleFavoritesButton').addEventListener('click', toggleFavorites);
...
function updateFavoritesList() {
    ...
    favorites.forEach((favorite, index) => {
        ...
        // 클릭 이벤트 추가하여 경로 검색
        li.addEventListener('click', () => loadFavorite(index));
        ...
        deleteBtn.addEventListener('click', (e) => {
            e.stopPropagation();  // 클릭이 li로 전달되지 않도록 방지
